...
export const gameQueue = new Queue("game-events", {
...
export const addGameFinishedJob = async (roomId: string) => {
	...
	try {
		...
		console.error(
			`[QueueService]  Failed to add job for room ${roomId}`,
			error,
		);
		throw error;
	}
...
console.log(
	`[QueueService] Initialized gameQueue with connection to ${process.env.REDIS_HOST || "localhost"}`,
);
