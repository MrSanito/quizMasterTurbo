...
export async function joinGame(
	...
	console.log(
		` [GameService] Constructed Leaderboard for ${userId}:`,
		leaderboard,
	);

	const payload: any = {
		gameState: stateData.status,
		myScore: parseInt(score || "0", 10),
		leaderboard, //  Send initial leaderboard
	};

	if (stateData.status === "PLAYING") {
		// Return current Question & Time Left
		const qIndex = parseInt(stateData.currentQuestionIndex || "0", 10);
		...
		if (questionStr) {
			...
			const options = Option
				? Option.map((o: any) => o.text)
				: currentQ.options || [];

			// Calculate Time Left
			const questionStartTime = parseInt(
				stateData.questionStartTime || "0",
				10,
			);
			...
			const myAnswer = await redisClient.hget(
...
export async function startGameLoop(roomId: string, io: Server) {
	...
	if (!acquired) {
		console.warn(
			` [GameService] Loop locked for ${roomId}. Skipping duplicate start.`,
		);
		return;
	}
	...
	if (questionCount === 0) {
		console.error(
			` [GameService] No questions found for room ${roomId}. Aborting loop.`,
		);
		io.to(roomId).emit("error", {
			message: "Game Error: No questions loaded.",
		});
...
async function nextQuestion(roomId: string, io: Server, index: number) {
	...
	if (!isLocked) {
		console.warn(
			` [GameService] Loop lock missing for ${roomId}. Stopping loop.`,
		);
		return;
	}
	...
	try {
		...
		console.log(
			` [GameService] Loaded ${questions.length} questions for ${roomId}`,
		);
	} catch (err) {
	...
	if (index >= questions.length) {
		console.log(
			` No more questions (Index ${index} >= ${questions.length}). Finishing game.`,
		);
	...
	setTimeout(
		() => endQuestion(roomId, io, index, question),
		QUESTION_TIME * 1000,
	);
}
...
async function endQuestion(
	...
	const correctText = correctOption
		? correctOption.text
		: question.correct || question.correctOptionId;
...
export async function submitAnswer(
	io: Server,
	data: { roomId: string; userId: string; answer: string; timeTaken: number },
) {
	...
	// 3. Get Question
	const index = parseInt((state.currentQuestionIndex as string) || "0", 10);
	const questions = await redisClient.lrange(`${roomKey}:questions`, 0, -1);
	const questionStr = questions[index];
	if (!questionStr) {
	...
	console.log(
		` [GameService] Saving answer for ${userId} to key: ${answersKey}`,
	);
	...
	console.log(
		` [GameService] Fetching player ${userId} from Redis:`,
		playerStr,
	);
	...
	io.to(roomId).emit("game:scoreUpdate", {
		userId,
		score: parseInt((newScore as string) || "0", 10),
		name,
	});
