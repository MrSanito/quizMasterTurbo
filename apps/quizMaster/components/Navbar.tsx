...
import { ChevronDown, Menu, X, Trophy, Flame, Brain, Award, User, LogOut } from "lucide-react";
      ...
      <header className="sticky top-0 z-50 w-full bg-[#340C97]/90 backdrop-blur-md border-b border-[#5B32B4]/30 transition-all duration-300">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6 text-white">
          ...
          <div className="flex items-center gap-8">
            <Link href="/" className="flex items-center gap-0.5 text-xl font-bold text-white hover:opacity-90 transition-opacity">
              <img src="/q-logo.svg" alt="Q" className="h-9 w-9 object-contain" />
            ...
            <nav className="hidden md:flex items-center gap-1 text-sm font-medium">
              ...
              <div className="dropdown dropdown-hover relative">
                ...
                <ul
                  tabIndex={0}
                  className="dropdown-content menu z-50 mt-1 w-48 rounded-2xl bg-[#1b0654] border border-[#5B32B4]/40 p-2 text-white shadow-2xl backdrop-blur-xl"
                >
                  <li>
                    <Link href="/quiz/mode" className="hover:bg-[#340C97] px-3 py-2.5 rounded-xl flex items-center gap-2">
                      <Flame size={15} className="text-yellow-400" />
                      <div>
                        <div className="font-semibold text-xs">Daily Trivia</div>
                        <div className="text-[10px] text-slate-400">Play daily challenge</div>
                      </div>
                    </Link>
                  </li>
                  <li>
                    <Link href="/quiz/mode" className="hover:bg-[#340C97] px-3 py-2.5 rounded-xl flex items-center gap-2">
                      ...
                      <div>
                        <div className="font-semibold text-xs">Multiplayer Arena</div>
                        <div className="text-[10px] text-slate-400">Battle live players</div>
                      </div>
                    </Link>
                  </li>
                </ul>
              ...
              <div className="dropdown dropdown-hover relative">
                ...
                >
                  <li>
                    <Link href="/categories" className="hover:bg-[#340C97] px-3 py-2.5 rounded-xl flex items-center gap-2">
                      <Brain size={15} className="text-indigo-400" />
                      <div>
                        <div className="font-semibold text-xs">All Categories</div>
                        <div className="text-[10px] text-slate-400">Browse by topic</div>
                      </div>
                    </Link>
                  </li>
                  <li>
                    <Link href="/practice/custom" className="hover:bg-[#340C97] px-3 py-2.5 rounded-xl flex items-center gap-2">
                      <Flame size={15} className="text-amber-400" />
                      <div>
                        <div className="font-semibold text-xs">Practice Mode</div>
                        <div className="text-[10px] text-slate-400">Custom practice questions</div>
                      </div>
                    </Link>
                  </li>
                </ul>
          ...
          <div className="hidden md:flex items-center gap-4 text-sm font-medium">
            {isLogin ? (
              <div className="flex items-center gap-3">
                ...
                >
                  <Avatar
                    src={user?.avatar ? `/avatars/${user.avatar}` : "/avatars/avatar1.svg"}
                    sx={{
                      width: 32,
                      height: 32,
      ...
      >
        <div>
          ...
          <div className="flex items-center justify-between mb-6">
            <Link href="/" onClick={() => setIsOpen(false)} className="flex items-center gap-0.5 text-lg font-bold text-white">
              <img src="/q-logo.svg" alt="Q" className="h-8 w-8 object-contain" />
              <span>uizMaster</span>
            </Link>
            <button
          ...
          {isLogin && user && (
            ...
            >
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-xs text-indigo-300 opacity-80">View Profile</p>
              </div>
            </Link>
          )}
