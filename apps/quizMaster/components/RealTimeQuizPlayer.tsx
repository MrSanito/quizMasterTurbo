...
export default function RealTimeQuizPlayer({
	...
	}, [initialTimeLeft]); // removed startTime to avoid flickering
				...
				<div className="flex flex-col gap-5 sm:gap-6 p-4 sm:p-5 md:p-8">
					...
					<div className="space-y-2">
						<div className="flex items-end gap-3">
							...
							<span
								className={`text-3xl sm:text-4xl font-bold leading-none tabular-nums transition-colors ${localTimeLeft <= 5 ? "text-error animate-pulse" : ""}`}
							>
								{localTimeLeft}
							</span>
						</div>
						<div className="w-full h-1.5 rounded-full bg-base-300 overflow-hidden">
							<div
								className={`h-full transition-all duration-1000 ease-linear ${localTimeLeft <= 5 ? "bg-error" : "bg-primary"}`}
								style={{ width: `${(localTimeLeft / initialTimeLeft) * 100}%` }}
							/>
						</div>
						...
						{instantFeedback && (
							<div
								className={`flex items-center gap-2 text-xs font-bold mt-1 ${instantFeedback.isCorrect ? "text-success" : "text-error"}`}
							>
					...
					<div className="space-y-2.5 mt-1">
						{options.map((option, index) => {
							...
							// Priority 4: Disabled if answered
								...
								stateClass =
									"bg-base-100 border-base-200 opacity-50 cursor-not-allowed";
							}
							...
							return (
								...
								>
									...
									{((correctAnswer && optionText === correctAnswer) ||
										(instantFeedback?.isCorrect && isSelected)) && (
									...
									{((correctAnswer &&
										...
										(instantFeedback &&
											!instantFeedback.isCorrect &&
											isSelected)) && <XCircle className="w-5 h-5 shrink-0" />}
