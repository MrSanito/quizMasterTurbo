...
const FriendRequest = () => {
					...
					prev.map((u) =>
						u.id === userId
							? {
									...
									friendshipId: res.data.friendRequest.id,
								}
							: u,
					),
				);
	...
	const handleDeclineRequest = async (requestId: string, isIncoming = true) => {
		...
		try {
			...
			if (res.data.success) {
				toast.info(
					isIncoming ? "Friend request declined" : "Friend request cancelled",
				);
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-6 border-b border-gray-100">
					<div>
						...
						<p className="text-gray-500 text-sm mt-1">
							Connect with fellow quiz takers, challenge them, and view your
							friends circle.
						</p>
					</div>
								...
								{friends.length === 0 ? (
									<div className="text-center py-12 border border-dashed border-gray-200 rounded-xl bg-gray-50">
										...
										<p className="text-gray-500 text-sm mt-1 max-w-sm mx-auto">
											Search for users in the "Add Friend" tab to expand your
											quiz circle.
										</p>
										<button
											onClick={() => setActiveTab("add")}
											...
											{incoming.map((item) => (
												...
												>
													...
													<div className="flex gap-2">
														...
														<button
															onClick={() =>
																handleDeclineRequest(item.requestId, true)
															}
															disabled={actionLoadingId === item.requestId}
										...
										<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
											{outgoing.map((item) => (
												...
												>
													...
													<button
														onClick={() =>
															handleDeclineRequest(item.requestId, false)
														}
														disabled={actionLoadingId === item.requestId}
													...
													>
														...
														<div>
															{user.friendshipStatus === "NONE" && (
																<button
																	onClick={() =>
																		handleSendRequest(user.username, user.id)
																	}
