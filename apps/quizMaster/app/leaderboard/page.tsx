...
const listUsers: LeaderboardUser[] = [
  { rank: 4, name: "Bhavna Mepani", points: 91, avatarSeed: "Bhavna-21", change: 2 },
      ...
      <div className="relative overflow-hidden rounded-[2.25rem] md:rounded-3xl border-[8px] md:border-4 border-violet-300 md:border-violet-500/20 shadow-2xl md:grid md:grid-cols-5 md:min-h-[550px]">
        ...
        <div className="relative bg-gradient-to-b from-[#4c3e7d] via-[#2b2153] to-[#160f2e] pb-10 pt-5 md:col-span-2 md:flex md:flex-col md:justify-between md:pb-14">
          ...
          >
            ...
            <polygon points="60,70 110,10 160,70" fill="#120b26" opacity="0.5" />
            <polygon points="150,70 190,25 230,70" fill="#120b26" opacity="0.6" />
            <polygon points="220,70 270,15 320,70" fill="#120b26" opacity="0.5" />
            <polygon points="300,70 350,22 400,70" fill="#120b26" opacity="0.6" />
          ...
          <div className="relative z-10 flex items-center justify-between px-4">
            <button aria-label="Back" className="text-white/90 transition hover:text-white">
              <ChevronLeft size={22} />
            </button>
                ...
                <div key={u.rank} className="flex flex-col items-center">
                  <div
                    className={`relative flex flex-col items-center justify-end rounded-t-[999px] pb-2 pt-3 ${
                      isFirst ? "h-[132px] w-[104px] bg-[#241a49]" : "h-[104px] w-[84px] bg-[#201a40]"
                    }`}
                  >
                    {isFirst && (
                      <Crown
                  ...
                  <p className="mt-1 text-sm font-medium text-white">{u.name}</p>
                  <p className={`font-bold text-white ${isFirst ? "text-xl" : "text-base"}`}>
          ...
          <ul className="divide-y divide-gray-100 px-4 md:px-8">
            {listUsers.map((u) => (
              <li key={u.rank} className="flex items-center gap-3 py-3 md:py-4">
                <span className="w-4 text-sm md:text-base font-medium text-gray-400">{u.rank}</span>
                <div className="avatar">
                ...
                <div className="flex-1">
                  <p className="text-sm md:text-base font-semibold text-slate-900">{u.name}</p>
                ...
                >
                  <span className="text-[10px]">{(u.change ?? 0) >= 0 ? "▲" : "▼"}</span>
