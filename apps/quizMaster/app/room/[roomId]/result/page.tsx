		...
		return (
			<div className="min-h-screen flex flex-col items-center justify-center gap-4">
				...
				<p className="animate-pulse opacity-70">
					Finalizing scores & analyzing results...
				</p>
			</div>
		);
	if (!resultData)
		return (
			<div className="p-10 text-center">
				No results found (or game not finished yet).
			</div>
		);

	// Find My Result
	const myResult =
		resultData.find((r: any) => r.user.id === user?.id) || resultData[0]; // Fallback to winner if viewing as guest?
	console.log(" My Result:", myResult);

	if (!myResult)
		return (
			<div className="p-10 text-center">
				Results parsed but user not found in list. Raw Results:{" "}
			...
			<div className="max-w-4xl mx-auto space-y-8">
							...
							<div className="flex items-center gap-4">
								<div className="avatar">
									<div className="w-20 h-20 rounded-full ring ring-primary ring-offset-base-100 ring-offset-2">
										<img
											src={`/avatars/${myResult.user.avatar || "avatar1.svg"}`}
										/>
									</div>
								</div>
								<div>
									<h2 className="text-2xl font-bold">
										{myResult.user.firstName}
									</h2>
				...
				<div className="space-y-4">
					...
					{myResult.answers.map((ans: any, i: number) => (
						<div
							key={i}
							className={`collapse collapse-arrow bg-base-100 border border-base-300 ${ans.isCorrect ? "border-l-4 border-l-success" : "border-l-4 border-l-error"}`}
						>
							<input type="checkbox" />
							...
							<div className="collapse-content space-y-2 text-sm">
								<div className="flex flex-col gap-1 p-3 bg-base-200/50 rounded-lg">
									...
									<span
										className={`${ans.isCorrect ? "text-success" : "text-error"} font-semibold`}
									>
								...
								{!ans.isCorrect && (
									<div className="flex flex-col gap-1 p-3 bg-success/10 rounded-lg border border-success/20">
										<span className="font-bold text-success opacity-80">
											Correct Answer
										</span>
