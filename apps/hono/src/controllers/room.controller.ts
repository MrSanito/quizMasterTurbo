...
export const createRoom = async (c: any) => {
	try {
...
export const getRoomResult = async (c: any) => {
	const roomIdParam = c.req.param("roomId");
	const roomId = Array.isArray(roomIdParam) ? roomIdParam[0] : roomIdParam;
	...
	try {
		...
		logger.error(error, "Get Room Result Error:");
		return c.json({ success: false, message: "Server error" }, 500);
	}
};

export const updateLobby = async (c: any) => {
	const roomIdParam = c.req.param("roomId");
	const roomId = Array.isArray(roomIdParam) ? roomIdParam[0] : roomIdParam;
	...
	try {
		...
		logger.error(error, "Lobby update error:");
		return c.json(
			{
				success: false,
				message: "Server error",
...
export const startRoom = async (c: any) => {
	const roomIdParam = c.req.param("roomId");
	const roomId = Array.isArray(roomIdParam) ? roomIdParam[0] : roomIdParam;
	...
	try {
		...
		logger.error(error, "Start room error:");
		return c.json({ success: false, message: "Server error" }, 500);
	}
};

export const finalizeRoom = async (c: any) => {
	const roomIdParam = c.req.param("roomId");
	const roomId = Array.isArray(roomIdParam) ? roomIdParam[0] : roomIdParam;
	...
	try {
		...
		logger.error(error, "Finalize game error:");
		return c.json({ success: false, message: "Server error" }, 500);
	}
};
