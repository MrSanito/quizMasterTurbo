...
import { computeJwkThumbprint, familyKey, generateAccessToken, generateRefreshToken, otpKey, rtKey, serverFingerprint, userCacheKey, verifyDpopProof, verifyRefreshToken } from "./auth.services";
...
export const verifyLoginOTP = TryCatch( async (req:Request , res:Response) => {
 // Rate limit: 5 wrong OTP attempts per 5 min per IP+email
    ...
    const { email, otp, deviceFingerprint, publicKeyJwk, browser, os, deviceType, deviceName } =
      parsed.data;
    ...
    // ── Device fingerprints ──────────────────────────────────────────────────
    // Client fingerprint = hash of browser signals (sent by client)
    // Server fingerprint = hash of IP + Accept headers (fallback)
    const combinedFingerprint = deviceFingerprint
      ? crypto
          .createHash("sha256")
          .update(deviceFingerprint + serverFingerprint(req))
          .digest("hex")
      : serverFingerprint(req);
...
export const refreshTokenController = TryCatch(async (req: Request, res: Response) => {
    ...
    if (session?.publicKeyThumbprint) {
      ...
      if (storedJwk) {
        const popValid = await verifyDpopProof({
          ...
          expectedMethod: "POST",
          expectedUrl: `${BASE_URL}/api/auth/refresh`,
          jtiCache: {
            has: async (jti) => {
  ...
  // 4. Device fingerprint drift check — soft signal, log only (don't hard-reject)
  const currentFingerprint = serverFingerprint(req);
  const session = await prisma.session.findUnique({
    where: { id: sessionId },
    select: { deviceFingerprint: true, expiresAt: true },
  });

  if (session?.deviceFingerprint) {
    // Server portion of stored fingerprint should match — extract it
    const storedServerPart = session.deviceFingerprint; // simplification; refine as needed
    if (!storedServerPart.includes(currentFingerprint.slice(0, 8))) {
      // Don't reject — fingerprints can drift (VPN, network change).
      // Log/alert instead. You can add a "suspicious" flag to Session here.
      console.warn(`[AUTH] Fingerprint drift for session ${sessionId}`);
    }
  }
...
export const validateUser = TryCatch(async (req: Request, res: Response) => {
  ...
  await redis.set(userCacheKey(userId), JSON.stringify(user), "EX", 5 * 60);
