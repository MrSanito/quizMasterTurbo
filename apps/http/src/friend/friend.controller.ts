...
export const sendRequest = TryCatch(async (req: Request, res: Response) => {
	...
	if (!username && !receiverId) {
		return res
			.status(400)
			.json({ success: false, message: "Username or receiverId is required" });
	}
	...
	if (existingRequest) {
		...
		return res
			.status(200)
			.json({ success: true, friendRequest: updatedRequest });
	}
...
export const listReceivedRequests = TryCatch(
...
export const listSentRequests = TryCatch(
	async (req: Request, res: Response) => {
		const { userId } = req.user;
		const requests = await prisma.friendRequest.findMany({
...
export const acceptRequest = TryCatch(async (req: Request, res: Response) => {
	...
	res
		.status(200)
		.json({ success: true, friendRequest: updatedRequest, friendship });
});
...
export const removeFriend = TryCatch(async (req: Request, res: Response) => {
	...
	if (!friendshipId) {
		return res
			.status(400)
			.json({ success: false, message: "Friendship ID is required" });
	}
...
export const friendshipStatus = TryCatch(
	async (req: Request, res: Response) => {
		const { userId } = req.user;
		...
		if (friendship) {
			return res
				.status(200)
				.json({ status: "ACCEPTED", friendshipId: friendship.id });
		}
