...
export async function getNonFriends(
  userId: string,
  options: { excludePending?: boolean; search?: string; skip?: number; take?: number } = {},
) {
  ...
  friendships.forEach((f) => excludeIds.add(f.userAId === userId ? f.userBId : f.userAId));
  ...
  if (excludePending) {
    ...
    pending.forEach((p) => excludeIds.add(p.senderId === userId ? p.receiverId : p.senderId));
  }
